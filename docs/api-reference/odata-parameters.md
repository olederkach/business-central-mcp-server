...
The Business Central MCP Server fully supports OData query parameters for efficient server-side filtering, sorting, and pagination of data.
...
All OData parameters are passed as arguments to the `list_records` tool.
...
- Use the `get_resource_schema` tool to inspect available fields
